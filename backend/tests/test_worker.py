...
@pytest.mark.asyncio
async def test_process_event_rejected_does_not_call_project_event():
    event_id = uuid.uuid4()
    agent_id = "sre-agent-212dbc"
    payload = VisionRequest(
        event_type="vision",
    ...
    with (
        patch("app.worker.evaluate", new=AsyncMock(return_value=(0.1, False, ["score too low"]))),
        patch("app.worker.mark_processed", new=AsyncMock(return_value=99)),
        patch("app.worker.commit_ops", new=AsyncMock()),
        patch("app.worker.apply_outcome", new=AsyncMock(return_value=0.55)),
        patch("app.worker.graph_projection.project_event", new=AsyncMock()) as project_event_mock,
    ):
        await process_event(pool, r, data)

    project_event_mock.assert_not_awaited()
